    ...
    <div className="flex items-start space-x-3 py-2 group">
      ...
      <div className="flex-1 bg-gray-100 dark:bg-gray-100 rounded-xl p-3 text-black">
        <div className="flex items-baseline space-x-2">
          <Link href={`/profile/${user.username}`}>
            <p className="font-semibold text-sm text-gray-100 dark:text-gray-800 hover:underline">
              {user.name || "Anonymous"}
            </p>
          </Link>
        ...
        <p className="text-sm text-gray-700 dark:text-gray-800 mt-1">
          {comment.text}
        </p>
      </div>
      {isOwner && (
...
export default function PostCard({ post }) {
  ...
  useEffect(() => {
    ...
    const channel = pusherClient.subscribe(`post-${post._id}`);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      // Save to DB
      const response = await axios.post(`/api/posts/${post._id}`, {
        text: newCommentText,
      });
            ...
            >
              ...
              <div className="flex-1">
                <textarea
                  ...
                  className="w-full p-2 border rounded-lg bg-gray-50 dark:bg-gray-200 dark:text-black"
                  rows="1"
                />
                <button
                  type="submit"
                  disabled={isSubmittingComment || !newCommentText.trim()}
                  className="mt-2 px-4 py-1.5 bg-blue-500 text-white rounded-lg text-sm font-semibold disabled:bg-blue-300 disabled:cursor-not-allowed"
                >
